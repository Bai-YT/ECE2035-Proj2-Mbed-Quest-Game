...
void* insertItem(HashTable* hashTable, unsigned int key, void* value) {
    ...
    // bkts = hashTable -> buckets;
    HashTableEntry* dup = findItem(hashTable, key);
    if(dup == NULL){
        int j = 0;
        while (j < (hashTable -> num_buckets)){
...
void* getItem(HashTable* hashTable, unsigned int key) {
    // bkts = hashTable -> buckets;
    // int* v, k;
    // while (int j < (hashTable -> num_buckets)){
    //    k = buckets[j];         // k is the address of the first item of the bucket
    //    if (value == (k -> value)
