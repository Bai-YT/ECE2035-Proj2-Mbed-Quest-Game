...
TEST(InitTest, SelfMade5)
{
    ...
    EXPECT_EQ(m[0], insertItem(ht, 0, m[0]));
...
TEST(AccessTest, GetKey_SelfMade1)
{
  HashTable* ht = createHashTable(hash, BUCKET_NUM);
  ...
  // Create a list of items to add to hash table.
    ...
    insertItem(ht, 5, m[5]);
    EXPECT_EQ(m[5], getItem(ht, 5));
    EXPECT_EQ(m[4], getItem(ht, 4));
    EXPECT_EQ(m[4], removeItem(ht, 4));
    insertItem(ht, 2, m[2]);
    EXPECT_EQ(m[2], removeItem(ht, 2));
    //deleteItem(ht, 2);
...
TEST(AccessTest, GetKey_SelfMade2)
{
    HashTable* ht = createHashTable(hash, BUCKET_NUM);
    ...
    // printHashTable(ht);
    EXPECT_EQ(NULL, insertItem(ht, 5, m[5]));
    EXPECT_EQ(m[2], insertItem(ht, 2, m[2]));
    EXPECT_EQ(m[2], removeItem(ht, 2));
    // printHashTable(ht);
    EXPECT_EQ(m[3], insertItem(ht, 3, m[3]));
    // printHashTable(ht);
    EXPECT_EQ(NULL, insertItem(ht, 4, m[4]));
...
TEST(AccessTest, GetKey_SelfMade3)
{
    HashTable* ht = createHashTable(hash, BUCKET_NUM);
...
TEST(AccessTest, GetKey_SelfMade4)
{
    HashTable* ht = createHashTable(hash, BUCKET_NUM);
    HashTable* ht2 = createHashTable(hash, BUCKET_NUM);
    ...
    //EXPECT_EQ(NULL, insertItem(ht, 2, m[80]));
    //EXPECT_EQ(NULL, insertItem(ht, 3, m[4]));
    //EXPECT_EQ(NULL, insertItem(ht, 4, m[3]));
//    EXPECT_EQ(m[3], insertItem(ht, 4, m[4]));
//    EXPECT_EQ(m[4], insertItem(ht, 3, m[3]));
//    EXPECT_EQ(m[3], insertItem(ht, 3, m[3]));
    //EXPECT_EQ(m[4], insertItem(ht, 4, m[3]));
//    EXPECT_EQ(NULL, insertItem(ht, 80, m[80]));
    //EXPECT_EQ(NULL, insertItem(ht, 81, m[80]));
    //deleteItem(ht, 80);
    //EXPECT_EQ(m[80], getItem(ht, 81));
    
    for(int i=0; i<1000; i++){
        EXPECT_EQ(NULL, insertItem(ht2, i, m[i]));
    }
    for(int j=999; j>-1; j--){
        EXPECT_EQ(m[j], getItem(ht2, j));
    }
    //EXPECT_EQ(m[50], insertItem(ht2, 50, m[46]));
    //EXPECT_EQ(m[46], insertItem(ht2, 50, m[46]));
    for(int j=99; j>-1; j--){
        deleteItem(ht2, j);
    }
    for(int j=150; j>99; j--){
        EXPECT_EQ(m[j], removeItem(ht2, j));
    }
    for(int j=99; j>55; j--){
        EXPECT_EQ(NULL, removeItem(ht2, j));
    }
    
//    EXPECT_EQ(NULL, insertItem(ht2, 50, m[50]));
//    EXPECT_EQ(m[50], insertItem(ht2, 50, m[46]));
//    EXPECT_EQ(m[46], getItem(ht2, 50));
